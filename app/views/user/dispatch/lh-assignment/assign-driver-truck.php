...
<script type="text/javascript">
    ...
    function show_list(){
        ...
        $.ajax({
            url:'../user/dispatch/lh-assignment/driver-wise-un-assigned-quick-list-ajax',
            type:'POST',
            data:{
                sort_by:$('#sort_by').val(),
...
<script type="text/javascript">
  $(document).ready(function(){
   $(document).on("click", "[data-action='choose-driver-truck-load']",function(){
      $.ajax({
        url:'../user/dispatch/lh-assignment/assign-driver-truck-action',
        type:'POST',
        data:{
          lha_id:$(this).data("lha-id"),
          load_id:check_url_params('id')
        },
        context: this,
        success:function(data){
         if((typeof data)=='string'){
