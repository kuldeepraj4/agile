...
<script type="text/javascript">
    function calender_days() {
        $.ajax({
            url: '../user/dispatch/lh-assignment/date-wise-total-loads-ajax',
            type: 'POST',
            data: {
...
<script type="text/javascript">
    ...
    function show_list() {
        ...
        $.ajax({
            url: '../user/dispatch/lh-assignment/load-wise-ajax',
            type: 'POST',
            data: {
                sort_by: $('#sort_by').val(),
...
is_high_priority_note = (item.high_priority_note == 'YES') ? '<span class="ic exclamation" style="color:yellow;text-shadow: 2px 2px #FF0000;"></span>' : '';
                    ...
                    show_t=(item.allocated_is_team_driver=='TEAM')?'T':'';
                        ...
                        <a class="text-link" href="../user/dispatch/loads/details?eid=` + item.eid + `">${is_high_priority_note} ${item.id}</a></td>
                        <td style="white-space:nowrap">${item.tentative_start_date} <i onclick="open_child_window({url:'../user/dispatch/loads/update-tentative-start-date?id=${item.id}&eid=${item.eid}&tentative-start-date=${item.tentative_start_date}',width:500,height:500,name:'update-ex-load-tentative-start-date'})" class="ic edit" title="Edit tentative start date"></i></td>
                        ...
                        <td style="white-space:nowrap" class="">${delivery_time}</td>
                        <td>${item.pick_up_region}</td>
                        <td>${item.status_id}</td>
                        <td>Trailer</td>
                        <td style="white-space:nowrap;background:white">
                        <span class="text-link"  onclick="open_quick_view_driver('${item.driver_eid}')">${item.driver_display_name}</span>`


                            if (item.driver_b_eid != "") {
                                row += `<br><span class="text-link"  onclick="open_quick_view_driver('${item.driver_b_eid}')">${item.driver_b_display_name}</span>`
                            }
                            if (item.lha_status == 'ASSIGNED' || item.lha_status == 'IN-TRANSIT') {
                                ...
                                row += `<i onclick="open_child_window({url:'../user/dispatch/lh-assignment/assign-driver-truck?id=` + item.id + `&driver-start-date=${item.tentative_start_date}',width:1200,height:500})" class="ic edit" title="Edit assigned driver/truck"></i>`
                            }
                            row += `</td>`
                            if (item.truck_id != "") {
                                row += `<td style="background:white"><span class="text-link"  onclick="open_quick_view_truck('${item.truck_eid}')">${item.truck_code}</span></td>
                            <td style="background:white">${item.truck_location}<br>${item.truck_location_updated_on}</td>`
                            } else {
                                row += `<td></td><td></td>`;
                            }
                            ...
                            if (item.lha_status == 'IN-TRANSIT' || item.lha_status == 'ASSIGNED') {
                                row += ` <i onclick="open_child_window({url:'../user/dispatch/lh-assignment/update-driver-started-at-datetime?eid=${item.lha_eid}&driver-started-date-time=${item.driver_started_at_datetime}',width:700,height:400})" class="ic edit" title="Update driver started datetime"></i>`
                            }
                            row += `</td>`
