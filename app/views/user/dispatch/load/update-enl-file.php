...
<section class="form-a" style="max-width: 600px;margin:auto;">
  <div class="form-a-header">Update Earning & Losses Receipt</div>
  ...
  function add_new(){
   show_processing_modal();
    ...
    if(isValidForm){
      ...
      if(jQuery.inArray(image_extension,['gif','jpg','pdf','jpeg','png','']) == -1){
        alert("Invalid image file");
      }

      var form_data = new FormData();
      form_data.append(`document`,property);
      form_data.append(`earning_losses_eid`,'<?php echo $data['eid']; ?>');
      $.ajax({
        url:'../user/dispatch/loads/update-enl-file-action',
        method:'POST',
        data:form_data,
        contentType:false,
        cache:false,
