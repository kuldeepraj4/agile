...
<!--         <div class="filter-item fourth">
        ...
        <div class="filter-item fourth">
            <label>Dispatch Completion</label>
            <select data-filter="dispatch_completion_status" onchange="set_params('dispatch_completion_status', this.value), goto_page(1),show_group_list()">
                <option value="">- - Select - -</option>
...
<script type="text/javascript">
    ...
    $(`[data-filter='dispatch_completion_status'] option[value="${check_url_params('dispatch_completion_status')}"]`).prop('selected', true);
     ...
     function show_list(){
        ...
        $.ajax({
            ...
            data:{
                ...
                //booked_by:check_url_params('booked_by_id'),
                is_team_driver:check_url_params('is_team_driver'),
                dispatch_completion_status:check_url_params('dispatch_completion_status'),
                ...
                if((typeof data)=='string'){
                   ...
                   if(data.status){
                     ...
                     if(hide_row_content==false){
                        ...
                        var row = `<tr id="row${item.load_id}">`  
                    }
                    ...
                    if(hide_row_content==false){
                        row +=`
                        <td class="${get_load_row_bg(item.load_status_id)}">${item.load_status_id} <br> <span style="white-space:nowrap;font-size:.8em">P ${item.load_picked_ratio} &nbsp D ${item.load_delivered_ratio}</span></td>
                    ...
                    <td></td>
                    <td>${item.load_tracking_status} <br>${item.load_tracking_updated_on} <i class="ic edit" style="color:grey" title="Update Tracking"  onclick="open_child_window({url:'../user/dispatch/loads/update-tracking&eid=${item.load_eid}&tracking-status=On Time',width:900,height:700})"></i></td>
