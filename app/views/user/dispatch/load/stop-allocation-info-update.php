...
<section class="lg-form-outer">
  <div class="lg-form-header">Update Allocation Information of Load <?php echo $details['load_id']; ?></div>
...
<script type="text/javascript">
  function save(){
    ...
    if(isValidForm){
     ...
     $.ajax({
      url:'../user/dispatch/loads/stop-allocation-info-update-action',
      type:'POST',
      data: obj,
...
<script type="text/javascript">
  function show_reason_of_cancellation(sel_status){
      ...
      if(sel_status=='CANCELLED' || sel_status=="BOUNCED"){
        get_dispatch_load_cancellation_reason({group:sel_status}).then(function(data) {
  // Run this when your request was successful
  if(data.status){
    //Run this if response has list
