...
<section class="rv content-box" style="margin: auto;max-width: 1600px">
  <h1 class="rv-heading">Empty Movements</h1>
...
<script type="text/javascript">
  function show_list() {
    $.ajax({
      url:'../user/dispatch/loads/empty-movements-ajax',
      type: 'POST',
      data: {
        sort_by: $('#sort_by').val(),
      ...
      success: function(data) {
        if ((typeof data) == 'string') {
          ...
          if (data.status) {
           ...
           <td>${item.driver_name} ${(item.is_team_driver=='TEAM')?`<b>T</b><br>${item.driver_b_name}`:''}</td>
           ...
           <td><span class="text-link" style="font-weight:bolder" onclick="open_child_window({url:'../user/dispatch/loads/empty-movements/update?eid=${item.eid}',width:1000,height:500,name:'Update Empty Movement'})">Update</span></td>`;
              row += `</tr>`;
