...
<section class="form-a" style="max-width: 600px">
  <div class="form-a-header">UPDATE VEHICLE MODEL</div>
  <form method="POST" id="MyForm" onsubmit="return update()">
    <input type="hidden" name="update_eid" value="<?php echo $data['eid']; ?>">
    <div>
      <label>Name</label>
      <input type="text" name="name" pattern="[a-zA-Z 0-9-]{1,}" required="required" value="<?php echo $details['name'] ?>">
    </div>
    <div>
      <label>Vehicle</label>
 ...
 get_vehicles().then(function(data) {
  ...
  if(data.status){
    ...
    if(data.response.list){
        ...
        $('[name="vehicle_id"] option[value="<?php echo $details['vehicle_id']; ?>"]').prop('selected',true);     
    }
  }
}).catch(function(err) {
...
show_makers({vehicle_id:'<?php echo $details['vehicle_id']; ?>'});
