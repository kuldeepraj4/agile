...
<br><br>
            ...
            <h1 class="list-200-heading">Ownership Types</h1>
    ...
    <section class="list-200-top-action">
        ...
        <div class="list-200-top-action-right">
            ...
            if(in_array('P0038', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/masters/vehicles/ownership-types/add-new'>Add New</a></button>";
            }
            ?>
        </div>
        </div>
...
counter++;
    ...
    row+=`<td>`+item.name+`</td>`;
    row+=`<td>`;
<?php if(in_array('P0040', USER_PRIV)){
    ?>
    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/masters/vehicles/ownership-types/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
    <?php
} ?>
<?php if(in_array('P0041', USER_PRIV)){
    ?>
...
$(document).ready(function(){
 $(document).on("click", "[data-action='delete']",function(){
    if(confirm('Do you want to delete ownership type ?')){
