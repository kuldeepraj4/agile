...
<br><br>
        <section class="form-a" style="max-width: 600px">
            <div class="form-a-header">UPDATE MOBILE COUNTRY CODE</div>
            <form method="POST" id="MyForm" onsubmit="return update()">
                    <input type="hidden" name="update_eid" value="<?php echo $data['eid']; ?>">
                <div>
                    <label>Prefix</label>
                    <input type="text" name="name" pattern="[0-9]{1,}" required="required" value="<?php echo $details['name'] ?>">
                </div>
                <div>
...
function back_alert() {
    ...
    $.ajax({
        ...
        success:function(data){
               ...
               alert(data.message);
               if(data.status){
                location.href="../user/masters/mobile-country-codes"
