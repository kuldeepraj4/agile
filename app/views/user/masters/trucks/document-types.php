...
<section class="list-200 content-box" style="margin: auto;max-width: 1000px">
    <h1 class="list-200-heading">Truck Document Types</h1>
    ...
    <section class="list-200-top-action">
        ...
        <div class="list-200-top-action-right">
            ...
            if(in_array('P0156', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/masters/trucks/document-types/add-new'>Add New</a></button>";
            }
            ?>
        </div>
        </div>
         ...
         if(data.status){
           ...
           $.each(data.response.list, function(index, item) {
            ...
            if(item.expiry_option){
             ...
             <?php if(in_array('P0158', USER_PRIV)){
              ?>
             row+=`<button title="Edit" class="btn_grey_c"><a href="../user/masters/trucks/document-types/update?eid=${item.eid}"><i class="fa fa-pen"></i></a></button>`;
              <?php
            }
            if(in_array('P0159', USER_PRIV)){
              ?>
