...
$truck_name=isset($_GET['truck-name'])?$_GET['truck-name']:'';
$truck_code=isset($_GET['truck-code'])?$_GET['truck-code']:'';
?>

<br><br>
<section class="list-200 content-box" style="margin: auto;max-width: 1300px">
  <h1 class="list-200-heading">Document History </h1>
  <p style="text-align:center;font-size:1em;"><?php echo $truck_code.' '.$truck_name ?></p>
...
<script type="text/javascript">
  function show_list(){
    $.ajax({
      ...
      type:'POST',
      data:{
        truck_eid:'<?php echo $data['truck_eid'] ?>',
         ...
         if(data.status){
           ...
           $.each(data.response.list, function(index, item) {
            $('#tabledata').append(`
              ...
              <td><button title="Notes" class="btn_grey_c" onclick="open_notes({url:'../user/miscellaneous/notes/details?reference=truck-DOCUMENT&eid=<?php echo $data['truck_eid'] ?>&document-type-eid=${item.type_eid}'})"><i class="fa fa-sticky-note"></i></a></button></td>
              <tr>
              `);
