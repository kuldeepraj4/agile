		...
		<section class="section-111" style="max-width: 700px"> 
			...
			<div>
              ...
              <div class="field-p">
                ...
                <input style="width:70px" type="text" list="quick_list_drivers" value="<?php echo $details['driver_code'].' '.$details['driver_name'] ?>" data-selected-driver-id="<?php echo $details['driver_id'] ?>" data-search-driver  name="driver_id">
              </div>
              <div class="field-p" data-driver-b>
                ...
                if($details['is_team_driver']=="TEAM"){
                  ...
                  <input style="width:70px" type="text" list="quick_list_drivers" data-selected-driver-id="<?php echo $details['driver_b_id'] ?>" value="<?php echo $details['driver_b_code'].' '.$details['driver_b_name'] ?>" data-search-driver  name="driver_b_id">
                  <?php
                }
                ?>
              </div>
              ...
              <div class="field-p">
                <label>Trailer</label>
                <input style="width:70px" type="text"  list="quick_list_trailers" value="<?php echo $details['trailer_code'] ?>" data-selected-truck-id="<?php echo $details['trailer_id'] ?>" name="trailer_id">
              </div>
							<div class="field-p">
								<label>Reason*</label>
								<select name="reason_id" data-default-select="<?php echo $details['reason_id'] ?>" required></select>
							</div>

							<div class="field-p">
								<label>Location (State/City)</label>
								<select name="state_id" style="width:150px" data-default-select="<?php echo $details['state_id'] ?>" onchange="show_cities({state_id:this.value})"></select>
								<select name="city_id" style="width:150px"></select>
							</div>

							<div class="field-p">
								<label>From Datetime*</label>
								<input type="text" name="from_date" data-date-picker value="<?php echo date('m/d/Y',strtotime($details['from_datetime'])) ?>">
								<input type="text" name="from_time" style="width: 100px;" data-time-picker value="<?php echo date('H:i',strtotime($details['from_datetime'])) ?>" >
							</div>

							<div class="field-p">
								<label>To Datetime*</label>
								<input type="text" name="to_date" data-date-picker value="<?php echo date('m/d/Y',strtotime($details['to_datetime'])) ?>">
								<input type="text" name="to_time" style="width: 100px;" data-time-picker value="<?php echo date('H:i',strtotime($details['to_datetime'])) ?>">
							</div>

							<div class="field-p">
								<label>Remarks</label>
								<textarea name="remarks" style="height: 100px"><?php echo $details['remarks'] ?></textarea>
							</div>
...
function back_alert() {
	...
	function save(){
		...
		if(isValidForm){
			...
			$.ajax({
				url:'../user/masters/drivers/drivers-leave-update-action',
				type:'POST',
				data: {
  ...
  if(data.status){
    ...
    if(data.response.list){
      var options="";
        ...
        select_default($('[name="state_id"]'));
        show_cities({state_id:'<?php echo $details['state_id']; ?>',default_select:'<?php echo $details['city_id']; ?>'})      
    }
  }
}).catch(function(err) {
...
function show_cities(param){
		$('[name="city_id"]').html(``);
	if(param.state_id!=''){
		 get_cities({state_id:param.state_id}).then(function(data) {
  // Run this when your request was successful
  if(data.status){
    //Run this if response has list
    if(data.response.list){
      var options="";
        ...
        if(param.hasOwnProperty('default_select')){
        	$(`
        		[name="city_id"] option[value="${param.default_select}"]`).prop('selected',true);
        }     
    }
  }
}).catch(function(err) {
