...
<section class="list-200 content-box" style="margin: auto;max-width: 700px">
    <h1 class="list-200-heading">Driver Teams</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0365', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/masters/driver-teams/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
...
<script type="text/javascript">
  ...
  function show_list(){
   ...
   $.ajax({
      url:'../user/masters/driver-teams/list-ajax',
      type:'POST',
      data:{
       ...
       if(data.status){
          ...
          $.each(data.response.list, function(index, item) {
             ...
             if(in_array('P0367', USER_PRIV)){
                ?>
                row+=`<button title="Edit" class="btn_grey_c"><a href="../user/masters/driver-teams/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                <?php
            }
            if(in_array('P0368', USER_PRIV)){
                ?>
    ...
    $(document).ready(function(){
       $(document).on("click", "[data-action='delete']",function(){
        if(confirm('Do you want to delete ?')){
            ...
            $.ajax({
              url:'../user/masters/driver-teams/delete-action',
              type:'POST',
              data:{
                delete_eid:eid
            },
