...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Salary Parameters</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0130', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/masters/employees/salary-parameters/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    // if(item.name.toUpperCase().includes(value) || item.parameter_type_id.toUpperCase().includes(value)){
        counter++;
        var row=``;
        row+=`<tr>`;
        row+=`<td>`+item.sr_no+`</td>`;
        row+=`<td style="text-align:center">`+item.name+`</td>`;
        row+=`<td style="text-align:center">`+item.parameter_type_id+`</td>`;
        row+=`<td>`;
        <?php if(in_array('P0132', USER_PRIV)){
            ?>
            row+=`<button title="Edit" class="btn_grey_c"><a href="../user/masters/employees/salary-parameters/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
            <?php
        }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
       $(document).on("click", "[data-action='delete']",function(){
        if(confirm('Do you want to delete residency ?')){
