...
<br><br>
        <section class="form-a" style="max-width: 600px">
            <div class="form-a-header">UPDATE RESIDENCY</div>
            <form method="POST" id="MyForm" onsubmit="return update()">
                    <input type="hidden" name="update_eid" value="<?php echo $data['eid']; ?>">
                <div>
                    <label>Type</label>
                    <select name="parameter_type_id" required="required" id="parameter_type_id"></select>
                </div>                    
                <div>
...
function back_alert() {
    ...
    $.ajax({
        ...
        success:function(data){
               ...
               alert(data.message);
               if(data.status){
                location.href="../user/masters/employees/salary-parameters"
...
<script type="text/javascript">
function show_parameter_type_id(){
 get_salary_parameter_types().then(function(data) {
  ...
  if(data.status){
    ...
    if(data.response.list){
      var options="";
        ...
        $('#parameter_type_id').html(options);
         $('#parameter_type_id option[value="<?php echo $details['parameter_type_id']; ?>"]').prop('selected',true);     
    }
  }
}).catch(function(err) {
