...
<section class="list-200 content-box" style="margin: auto;max-width: 1000px">
    <h1 class="list-200-heading">Trailer Document Types</h1>
        ...
        <div class="list-200-top-action-right">
            <div>
                ...
                if(in_array('P0168', USER_PRIV)){
                    echo "<button class='btn_grey button_href'><a href='../user/masters/trailers/document-types/add-new'>Add New</a></button>";
                }
                ?>
            </div>
        </div>
             ...
             $.each(data.response.list, function(index, item) {
                // if(item.name.toUpperCase().includes(name)){       
                  ...
                  <?php if(in_array('P0170', USER_PRIV)){
                      ?>
                      row+=`<button title="Edit" class="btn_grey_c"><a href="../user/masters/trailers/document-types/update?eid=${item.eid}"><i class="fa fa-pen"></i></a></button>`;
                      <?php
                  }
                  if(in_array('P0171', USER_PRIV)){
                      ?>
