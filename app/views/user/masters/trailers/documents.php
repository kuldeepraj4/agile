...
<section class="list-200 content-box" style="margin: auto;max-width: 1300px">
  <h1 class="list-200-heading">Trailer Documents <?php echo (isset($_GET['trailer_code']))?$_GET['trailer_code']:''; ?></h1>
  ...
  <section class="list-200-top-action">
        <div class="list-200-top-action-left">
            ...
            <input type="text" data-filter="code" name="quick_list_trailers_search" list="quick_list_trailers">
            </div>
...
<script type="text/javascript">
  ...
  if (url_params.hasOwnProperty('trailer_code')) {
    $("[data-filter='code']").val(url_params.trailer_code);
  }



  $(document.body).on('change', '[name="quick_list_trailers_search"]' ,function(){
    if($('[name="quick_list_trailers_search"]').val() !== ""){
    Trailer_eid_selected=$(`[data-Trailer-filter-rows="${$(this).val()}"]`).data('value');
    Trailer_code_selected=$(`[data-Trailer-filter-rows="${$(this).val()}"]`).data('code');
    if(Trailer_eid_selected!=undefined){
      location.href=`user/masters/trailers/documents?eid=${Trailer_eid_selected}&trailer_code=${Trailer_code_selected}`

      //$(this).data('selected-Trailer-id',Trailer_id_selected)
    }else{
      alert("Please enter correct Trailer ID");
      $('[name="quick_list_trailers_search"]').val("");
    }
  }
  });
  ...
  if(data.status){
    ...
    if(data.response.list){
      ...
      $.each(data.response.list, function(index, item) {
        ...
        options+=`<option data-Trailer-filter-rows="`+item.code+`" data-value="${item.eid}" data-name="${item.code}" data-code="${item.code}" value="`+item.code+`"></option>`;              
        } 
...
<script type="text/javascript">
  function show_list(){
    var trailer_eid='<?php echo $data['trailer_eid'] ?>'
    $.ajax({
      url:location.pathname+'-ajax',
      type:'POST',
      data:{
        
        trailer_eid:trailer_eid
      },
           ...
           $.each(data.response.list, function(index, item) {
              ...
              if(item.is_uploaded){
               ...
               <?php if(in_array('P0145', USER_PRIV)){
                ?>
                row+=`<button title="Edit" class="btn_grey_c"><a href="../user/masters/trailers/documents/upload?trailer_eid=<?php echo $data['trailer_eid']; ?>&document_eid=${item.type_eid}&document_name=${item.type_name}"><i class="fa fa-upload"></i></a></button>`;
                <?php
              } ?>
              ...
              <td></td>`
               ...
               if(item.document_details.remarks != undefined && item.document_details.remarks != null && item.document_details.remarks != ''){
                row+=`<td>${remarks}</td>`;
              }else{
            ...
            <button title="History" class="btn_grey_c"><a href="../user/masters/trailers/documents/document-history?trailer_eid=<?php echo $data['trailer_eid']; ?>&document_type_eid=${item.type_eid}&document_name=${item.type_name}"><i class="fa fa-history"></i></a></button>
            <button title="Notes" class="btn_grey_c" onclick="open_notes({url:'../user/miscellaneous/notes/details?reference=TRAILER-DOCUMENT&eid=<?php echo $data['trailer_eid']; ?>&document-type-eid=${item.type_eid}'})"><i class="fa fa-sticky-note"></i></a></button></td>`
