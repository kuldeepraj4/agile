...
<section class="list-200 content-box" style="margin: auto;">
  <h1 class="list-200-heading">Dashboard - Unschedule</h1>
  ...
  <section class="list-200-top-action">
    ...
    <div class="list-200-top-action-right">
      <div>
        <?php if (in_array('P0401', USER_PRIV)) {
          echo "<button class='btn_grey button_href'><a onclick='update_records_for_view()'>Update Unschedule</a></button>";
        } ?>
      </div>
    </div>
  </section>
...
<script>
  $(document.body).on('change', "[data-filter='followup_date']", function() {
    ...
    if (g1.getTime() > g2.getTime()) {
      alert("Followup Date should be less than from Followup Date (<=) ")
  ...
  $(document.body).on('change', "[data-filter='followup_end_date']", function() {
    ...
    var g2 = new Date(check_url_params('followup_end_date'))
    if (g1.getTime() > g2.getTime()) {
      alert("Followup Date should be greater than from Followup Date (<=)")
          ...
          if (data.status && unit_type_id) {
            $.each(data.response.list, function(index, item) 
            {
             ...
             /* if(check_url_params('followup_by')){
                if(!item.follow_up_user_name.includes(followup_by)){
                  return;
                }
              }*/
              ...
              if(item.un_repairorder_id != '0'){
             row += ` <td class="darkblue"><span class="text-link"><a style="color:white;" href="../user/maintenance/repair-orders/details?eid=${item.un_repairorder_eid}">${item.un_repairorder_id}</span></td>`;
             sr_no_count++;
           }else{
            row += ` <td ></td>`;
           }
             ...
             <td>${item.un_tat}</td>
              ...
              <td>${item.un_type}</td>`;
              if (item.critical.indexOf('HIGH') >= 0) {
                row += `<td style="background-color:#ffcce0">HIGH</td>`;
              } else if (item.critical.indexOf('HIGH') <= -1 && item.critical.indexOf('MEDIUM') >= 0) {
                row += `<td style="background-color:#fff0b3">MEDIUM</td>`;
              } else {
                row += `<td style="background-color:white">${item.critical}</td>`;
              }
              row += `
             <td style="min-width:150px;text-align:left">${item.job_work}</td>
             <td class="lightblue" style="min-width:250px; padding:0px 0px;">${item.un_last_note}</td>
             <td class="lightblue">${item.un_next_followup}</td>`;
             row += `<td> ${item.follow_up_user_code} ${item.follow_up_user_name}<br> ${item.follow_up_next_datetime}</td>`;
              <?php
