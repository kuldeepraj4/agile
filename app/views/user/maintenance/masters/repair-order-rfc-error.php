...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Repair Order RFC's Reason</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0394', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/repair-order-rfc-error/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
    ...
    if(data.response.list){
        ...
        $.each(data.response.list, function(index, item) {
            // if(item.name.toUpperCase().includes(value) || item.class_name.toUpperCase().includes(value) || item.status.toUpperCase().includes(value)){
                ...
                row+=`<td>`;
                <?php if(in_array('P0396', USER_PRIV)){
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/repair-order-rfc-error/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
                <?php if(in_array('P0397', USER_PRIV)){
                    ?>
