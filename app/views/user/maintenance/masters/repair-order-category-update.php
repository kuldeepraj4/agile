...
<section class="form-a" style="max-width: 600px">
    <div class="form-a-header">UPDATE - REPAIR ORDER CATEGORY</div>
    <form method="POST" id="MyForm" onsubmit="return update()">
        ...
        <label>Status</label>
        <select name="status" id="status" data-filter="status" data-default-select="<?php echo $details['status']; ?>" required>
        ...
        if(isValidForm){
            ...
            $.ajax({
                ...
                success:function(data){
                 ...
                 alert(data.message);
                 if(data.status){
                    location.href="../user/maintenance/masters/repair-order-category"
