...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Repair Order Type</h1>
    ...
    <section class="list-200-top-action">
        <div class="list-200-top-action-left">
            ...
            <div class="filter-item-full">
                <label>Search</label>
                <input type="text" placeholder="name / class / status" data-filter="search" onkeyup="set_params('page_no', 1);show_list()">
            </select>
        </div>            
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0191', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/repair-order-type/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
  ...
  if(data.status){
    ...
    if(data.response.list){
        ...
        $.each(data.response.list, function(index, item) {
            // if(item.name.toUpperCase().includes(value) || item.class_name.toUpperCase().includes(value) || item.status.toUpperCase().includes(value)){
                counter++;
                var row=``;
                row+=`<tr>`;
                ...
                row+=`<td>`;
                <?php if(in_array('P0193', USER_PRIV)){
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/repair-order-type/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
                <?php if(in_array('P0194', USER_PRIV)){
                    ?>
