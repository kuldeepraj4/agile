...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Repair Order Stage</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0197', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/repair-order-stage/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
    ...
    if(data.response.list){
        ...
        $.each(data.response.list, function(index, item) {
            // if(item.name.toUpperCase().includes(value)){
                ...
                row+=`<td>`;
                <?php if(in_array('P0199', USER_PRIV)){
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/repair-order-stage/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
                <?php if(in_array('P0200', USER_PRIV)){
                    ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
     $(document).on("click", "[data-action='delete']",function(){
        if(confirm('Do you want to delete repair order stage ?')){
