...
<section class="list-200 content-box" style="margin: auto;max-width: 800px">
    <h1 class="list-200-heading">Incident Documents List</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0314', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/incident-documents/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
        ...
        $.each(data.response.list, function(index, item) {
          ...
          //console.log(data.response.list);
            ...
            // if(item.name.toUpperCase().includes(value)||item.status.toUpperCase().includes(value)){
                ...
                row+=`<td>`;
                <?php if(in_array('P0316', USER_PRIV)){
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/incident-documents/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
                <?php if(in_array('P0317', USER_PRIV)){
                    ?>
