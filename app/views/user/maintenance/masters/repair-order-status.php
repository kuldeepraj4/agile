...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Repair Order Status</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0088', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/repair-order-status/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
...
<script type="text/javascript">
  ...
  get_repair_order_status(param).then(function(data) {
        ...
        $.each(data.response.list, function(index, item) {
            // if(item.name.toLowerCase().includes(value)){
                ...
                <?php if(in_array('P0090', USER_PRIV)){
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/repair-order-status/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
       $(document).on("click", "[data-action='delete']",function(){
        if(confirm('Do you want to delete repair order status ?')){
