...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Fault Reason Truck's</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            if(in_array('P0287', USER_PRIV)){
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/fault-reason-trucks/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
...
<script type="text/javascript">
 ...
 let param={
    ...
    get_reasons_list_trucks(param).then(function(data) {
    ...
    if(data.response.list){
        ...
        $.each(data.response.list, function(index, item) {
            // if(item.name.toUpperCase().includes(value) || item.status.toUpperCase().includes(value)){
                ...
                row+=`<td>`;
                <?php if(in_array('P0289', USER_PRIV)){
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/fault-reason-trucks/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
                <?php if(in_array('P0290', USER_PRIV)){
                    ?>
