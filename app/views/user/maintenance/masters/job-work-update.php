...
<section class="form-a" style="max-width: 600px">
    <div class="form-a-header">UPDATE - JOB WORK MASTER</div>
    <form method="POST" id="MyForm" onsubmit="return update()">
        <input type="hidden" name="update_eid" value="<?php echo $data['eid']; ?>">
        <div>
            <label>Job Work Type Name</label>
            <!-- <input type="text" name="name" required="required" value="<?php echo $details['name'] ?>"> -->
            <input type="text" name="name" pattern="^[a-zA-Z0-9_-]+( [a-zA-Z0-9_-]+)*$" required="required" value="<?php echo $details['name'] ?>">
        </div><br>
        ...
        if(isValidForm){
            ...
            $.ajax({
                ...
                success:function(data){
                 ...
                 alert(data.message);
                 if(data.status){
                    location.href="../user/maintenance/masters/job-work"
