...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Job Work Type Master</h1>
    ...
    <div class="list-200-top-action-right">
        <div>
            ...
            {
                echo "<button class='btn_grey button_href'><a href='../user/maintenance/masters/job-work-type/add-new'>Add New</a></button>";
            }
            ?>
        </div>
    </div>
    ...
    if(data.response.list){
        ...
        $.each(data.response.list, function(index, item) 
        {
            ...
            // {
                ...
                row+=`<td>`;
                <?php if(in_array('P0209', USER_PRIV))
                {
                    ?>
                    row+=`<button title="Edit" class="btn_grey_c"><a href="../user/maintenance/masters/job-work-type/update?eid=`+item.eid+`"><i class="fa fa-pen"></i></a></button>`;
                    <?php
                } ?>
                <?php if(in_array('P0210', USER_PRIV))
                {
                    ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
       ...
       {
        if(confirm('Do you want to delete job work type ?'))
        {
