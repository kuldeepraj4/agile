...
<section class = 'list-200 content-box' style = 'margin: auto;max-width: 1200px'>
  ...
  <div class = 'table  table-a'>
    <table data-document-table>
      ...
      <tfoot>
       <tr><td colspan="3"></td><td style="padding: 4px;text-align: right;"><button type="button" class="btn_blue" onclick="open_child_window({url:'../user/maintenance/inspection-sheet/upload-document&eid=<?php echo $data['eid'] ?>',width:600,height:500})">Add Document</button></td></tr>
     </tfoot>
   </table>
   <!-- <div style="display: flex;justify-content: center;padding: 10px">

    <button class="btn_green" onclick="back_alert()">Back To List</button>
  ...
  $(document).ready(function() {
    $(document).on("click", "[data-action='delete']", function(){
      if (confirm('Do you want to delete docs?')){
        ...
        $.ajax({
          url: 'user/maintenance/inspection-sheet/delete-document-action',
          type: 'POST',
          data: {
