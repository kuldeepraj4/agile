...
<section class="form-a" style="max-width: 600px;margin:auto;">
  <div class="form-a-header">UPLOAD - INSPECTION DOCUMENT</div>
  ...
  function add_new(){
   show_processing_modal();
    ...
    if(isValidForm){
      ...
      if(jQuery.inArray(image_extension,['eml','gif','jpg','pdf','jpeg','png','','doc','docx','flv','mpeg4','mp4','avi','mov']) == -1){
        alert("Invalid image or video file");
        return;
      }

      var form_data = new FormData();
      form_data.append(`document`,property);
    form_data.append(`repair_order_eid`,'<?php echo $data['eid']; ?>');
    form_data.append(`remarks`,$('[name="remarks"]').val());
      $.ajax({
        url:'../user/maintenance/inspection-sheet/upload-document-action',
        method:'POST',
        data:form_data,
        contentType:false,
        cache:false,
