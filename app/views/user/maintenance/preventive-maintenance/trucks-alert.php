...
<section class="list-200 content-box" style="margin: auto;max-width: 1400px">
  <h1 class="list-200-heading">Preventive Maintenace Truck Alert</h1>
  ...
  <section class="list-200-top-action">
    <div class="list-200-top-action-left">
      ...
      <div class="filter-item" style="width:100%;">
        ...
        <!-- <select data-filter="job_work_name" onchange="set_params('job_work_name', this.value), goto_page(1)" multiple>
        </select> -->
      </div>
    <div class="filter-item"></div>
          ...
          $.each(data.response.list, function(index, item) {
           ...
           row = `<div class="checkbox-button">
            <label style="width: 30%; padding: 5px 8px; background: #f1f1f1; margin-right: 5px;">Criticality</label>
             <div class="checkbox-button-color "  style=" background: #ffcce0;">
              <input type="checkbox" id="HIGH" name="HIGH" value="HIGH" data-status-id-group checked>
                <label for="HIGH"> HIGH - <span>`+ item.high +`</span></label>
            </div>
            <div class="checkbox-button-color" style="background: #fff0b3;">
              ...
              <label for="MEDIUM" > MEDIUM - <span>`+ item.medium +`</span></label>
               </div>
              ...
              <label for="LOW"> LOW - <span>`+ item.low +`</span></label>
              </div>
            </div>
...
<div class="filter-item"></div>
  ...
  <div class="list-200-top-action-right">
    <div>
      <?php if (in_array('P0402', USER_PRIV)) {
        echo "<button class='btn_grey button_href'><a onclick='update_records_for_view()'>Update PM Truck Alert</a></button>";
      } ?>
    </div>
  </div>
  ...
  <div class="table  table-a">
    ...
    <table data-ro-table>
      <thead>
        <tr>
          <th>Sr. No.</th>
          <th data-table-sort-by="truck">Truck</th>
          <th data-table-sort-by="job_work">Job Work</th>
          <th data-table-sort-by="current_odo">Current Odo Meter</th>
          <th data-table-sort-by="current_engine_hours">Current Engine Hours</th>
          <th data-table-sort-by="last_work_order">Last Work Order</th>
          <th data-table-sort-by="wo_date">WO Date</th>
          <th data-table-sort-by="wo_odometer">WO Odometer</th>
          <th data-table-sort-by="wo_engine_hours">WO Engine Hours</th>
          <th>PM Value Actual (A)</th>
          <th>PM Value Temporary(B)</th>
          <th data-table-sort-by="pm_value">PM Value (A+B)</th>
          <th>PM Advance Alert Actual (X)</th>
          <th>PM Advance Alert Temporary (Y)</th>
          <th data-table-sort-by="pm_advance_alert">PM Advance Alert (X+Y)</th>
          <th data-table-sort-by="pm_mode">PM Mode </th> 
          <th data-table-sort-by="difference">Difference <br>(Current Value - Old Value)</th> 
          <th data-table-sort-by="criticality">Criticality</th>
          <th>Action</th>
        </tr>                       
      </thead>
...
<script type="text/javascript">
  ...
  $(function() {
    ...
    $("#btnSave").click(function() {
        ...
        if (PMValueTemparory){
            $.ajax({
              url:'../user/maintenance/preventive-maintenance/trucks-alert/addpmtemparory-action',
              type:'POST',
              data:{
...
<script type="text/javascript">
  $(document.body).on('input', '[data-vehicle-id]', function() {
    ...
    if (id_selected != undefined) {
      ...
      set_params('truck_name', $(`[data-vehicle-id]`).val())
      //goto_page(1)
    }
  });
</script>
    ...
    if (data.status) {
      ...
      if (data.response.list) {
        ...
        if (url_params.hasOwnProperty('truck_name')) {
          $(`[data-vehicle-id]`).val(check_url_params('truck_name'))
...
//console.log(active_status_id_array);

 
  function show_list(){
   console.log(options_selected_new);
       ...
       if(data.status){
        //console.log(data)
         ...
         if(criticality_level == 'HIGH')  {
            valuesDetails = data.response.list;
         } else if (criticality_level  == 'MEDIUM') {
            valuesDetails = data.response.list;
         } else if (criticality_level  == 'LOW') {
            valuesDetails = data.response.list;
         } else {
           valuesDetails = data.response.list;
         }
         $.each(valuesDetails, function(index, item) {
          ...
          if(item.criticality_level=='HIGH'){
           ...
           <td>${item.difference}</td>
           <td style="background:${criticality_bg}">${item.criticality_level}</td>
           <td><input type="button" class="button-3" value="Add PM Value Temporary" id="btn1" onClick="AddValuesForTemparory(${item.truck_id},${item.job_work_id})"></td>`;
