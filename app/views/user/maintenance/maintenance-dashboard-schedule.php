...
<section class="list-200 content-box" style="margin: auto;">
  <h1 class="list-200-heading">Dashboard - Schedule</h1>
  ...
  <section class="list-200-top-action">
    ...
    <!-- //input used for sory by call-->
     <input type='hidden' id='sort' value='asc'>
      ...
      <div class="filter-item">
        ...
        <select data-filter="unit_type_id" onchange="$(`[data-vehicle-id]`).val(''),set_params('unit_type_id', this.value),set_params('vehicle_code', ''),set_params('unit_id', ''),set_params('followup_by', ''), show_unit_filter({unit_type_id:this.value})"></select>
        <!-- Set param end  here-->
      </div>
      <div class="filter-item">
        <label>Unit ID</label>
    ...
    <div class="list-200-top-action-right">
      <div>
        <?php if (in_array('P0400', USER_PRIV)) {
          echo "<button class='btn_grey button_href'><a onclick='update_records_for_view()'>Update Schedule</a></button>";
        } ?>
      </div>
    </div>
  </section>
          ...
          if (data.status && unit_type_id) {
            $.each(data.response.list, function(index, item) 
            {
              ...
              /*if(check_url_params('followup_by')){
                if(!item.follow_up_user_name.includes(check_url_params('followup_by'))){
                  return;
                }
              }*/
            ...
            row += ` <td ></td>`;
              ...
              <td>${item.sc_type}</td>`;
              if (item.sc_critical.indexOf('HIGH') >= 0) {
                row += `<td style="background-color:#ffcce0">HIGH</td>`;
              } else if (item.sc_critical.indexOf('HIGH') <= -1 && item.sc_critical.indexOf('MEDIUM') >= 0) {
                row += `<td style="background-color:#fff0b3">MEDIUM</td>`;
              } else {
                row += `<td style="background-color:white">${item.sc_critical}</td>`;
              }
              row += `
             <td style="min-width:150px;text-align:left">${item.sc_job_work}</td>
             <td class="lightblue" style="min-width:250px; padding:0px 0px;">${item.sc_last_note}</td>
             <td class="lightblue">${item.sc_next_followup}</td>`;


              row += `<td> ${item.sc_follow_up_user_code} ${item.sc_follow_up_user_name}<br>${item.sc_follow_up_next_datetime}</td>`;
