  ...
  <section class="list-200-top-action">
    <div class="list-200-top-action-left">
      ...
      <div class="filter-item">
        ...
        <input type="text" data-filter="vendor_name" onchange="set_params('vendor_name', this.value), goto_page(1)">
        </select>
      </div>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th style="text-align: right;">Reconciliation</th>
        </tr>
      </thead>
...
<script type="text/javascript">
  ...
  if (url_params.hasOwnProperty('order_status_id')) {} else {
    set_params('order_status_id', 'PENDING')
  }
...
<script type="text/javascript">
  function show_status_filter_1() {
    get_workorder_approval_status().then(function(data) {
      if (data.status) {
...
<script type="text/javascript">
  function show_list() {
    ...
    // var status_id = $('[name="order_status_id"]').val();
    var status_id = check_url_params('order_status_id');
                ...
                if (payments_list_length > 0) {
                  ...
                  $.each(item.payments_list, function(index2, item2) {
                   ...
                   // if (row_span_condtion < 1) {
                    ...
                    <td rowspan="" data-driver-eid="${item.driver_eid}">${item.driver_code}</td>
                    ...
                    rows += `<td><select name="reconcilation_id" class="op" required>
...
//   var stop_row=
//   {
//     trip_id : $data_stop_row.find('[name="trip_id"]').text(),
//     reconcilation_id : $data_stop_row.find('[name="reconcilation_id"]').val(),
//   }
//   //console.log(stop_row.reconcilation_id)
...
<!-- <script type="text/javascript">
  async function f() 
  {
    var issue_list='<?php echo json_encode($list['payments_list']) ?>';
    issue_list=JSON.parse(issue_list)
    $.each(issue_list,function(index,item) 
    {
        default_select_status_id:item.status_id
    })
  }
  f()
</script> -->
