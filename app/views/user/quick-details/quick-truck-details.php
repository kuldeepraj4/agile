...
?>
<?php
require_once APPROOT . '/views/user/quick-details/quick-truck-excel.php';
?>
<!-- ------------------------------TRUCK BASIC INFORMATION SECTION START HERE------------------------------------------------------------------------------------------------------------- -->
<section class="lg-form-outer lg-form list-200 content-box">
    <section class="profile-ms-outer">
        <div class="profile-ms-header" style="margin-top:-23px; font-weight:bolder; color:brown;">TRUCK ID <?php echo $details['code']; ?></div>
<div>
<div class="dropdown" style="float:right; margin-right:10px;margin-top:-35px;">
  ...
  <a onclick="tableToExcel('personal', 'All TRUCKS DATA')" >All</a>
  <a onclick="tableToExcel('tabledata_document_excel', 'Documents Excel Data')">Truck Documents</a>
  <a onclick="tableToExcel('tabledata_rolist_excel', 'RO Excel Data')">Repair Order</a>
  <a onclick="tableToExcel('tabledata_wolist_excel', 'WO Excel Data')">Work Order</a>
  <a onclick="tableToExcel('tabledata_unsc_excel', 'Schedule/Unschedule Excel Data')">Schedule/Unschedule</a>
  </div>
</div>
        <div class="profile-ms">
            <section class="profile-ms-section-3" style="margin-bottom:-55px;">
                <div>
                    <fieldset>
                    ...
                    <fieldset>
                        ...
                        <div class="field-section">
                            ...
                            <div class="field">
                                ...
                                <div><?php echo $details['current_odometer_reading']; ?></div>
                            </div>
                            <div class="field">
                                <label>Odometer Updated On</label>
                                <div><?php echo $details['odometer_updated_on']; ?></div>
                            </div>
                            <div class="field">
...
<!-- ------------------------------TRUCK BASIC INFORMATION SECTION END HERE------------------------------------------------------------------------------------------------------------- -->
...
<script type="text/javascript">
    function show_list_rolist() {
        ...
        var vehicle_code = '<?php echo $details['code']; ?>';
        $.ajax({
...
<section class="list-200 content-box" style="margin: auto;max-width: 1400px">
    ...
    <div class="table  table-a">
        <input type='hidden' id='sort' value='asc'>
        <table data-ro-table-wolist id="tabledata_wolist_excel">
            <thead>
                <tr>
                    <th>Sr. No.</th>
...
<section class="list-200 content-box" style="margin: auto;">
    ...
    <div class="table  table-a">
        <input type='hidden' id='sort' value='asc'>
        <table data-ro-table-unsc id="tabledata_unsc_excel" style="font-size: 12px;">
            <thead>
                <tr>
                    <th>Sr. No.</th>
                    <th>Unit No.</th>
            ...
            success: function(data) {
              ...
              <td>${item.sr_no}</td>`;
                            ...
                            if (unit_type_id == "TRUCK") {
                                row += `<td><span>${item.unit_code}</span></td>
              ...
              <td>${item.current_location}</td>`;
                            if (item.sc_repairorder_id != '0') {
                                row += ` <td  style="color:white; background-color: #486e94;"><span class="text-link"><a style="color:white;">${item.sc_repairorder_id}</span></td>`;
                            } else {
                                row += ` <td ></td>`;
                            }
...
<!-- ---------------------ALL TRUCKS NOTES SECTION START HERE--------------------------------------------------------------------------------- -->
<br><br>
<style type="text/css">
    .notes-area {
        width: 95%;
...
<!-- ---------------------ALL TRUCKS NOTES SECTION ENDS HERE--------------------------------------------------------------------------------- -->

<!-- ------------------------------TRUCK DOCUMENTS SECTION START HERE------------------------------------------------------------------------------------------------------------- -->
<br>
<section class="lg-form-outer lg-form list-200 content-box">
    <section class="" style="margin: auto;max-width: auto">
        <h1 class="list-200-heading">Truck Documents</h1>
        <div class="table  table-a">
...
<script type="text/javascript">
    function show_list_document() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                truck_eid: check_url_params('eid')
            },
            success: function(data) {
                if ((typeof data) == 'string') {
                    data = JSON.parse(data)
              ...
              <td><span class='${required_option_class}'><span><span class='d-none'>${required_option_verify}</span></td>`
               ...
               <button class='btn_grey_c' onclick="open_document('${item.document_details.file_path}')"><i class='fa fa-file'></i></button>`
                                ...
                                ?>
                                    // row += `<button title="Edit" class="btn_grey_c"><a href="../user/masters/trucks/documents/upload?truck_eid=${dd}&document_eid=${item.type_eid}&document_name=${item.type_name}"><i class="fa fa-upload"></i></a></button>`;
                                <?php
                                } ?>
                  ...
                  // </td>`
                                ...
                                row += `<td>${item.document_details.added_by_user_code} <br>${item.document_details.added_by_user_name}<br> <span > ${item.document_details.added_on_datetime}</span></td>
              <td>${item.document_details.verified_by_user_code} <br> ${item.document_details.verified_by_user_name}  <br><span > ${item.document_details.verified_on_datetime}</span></td>
              <td>${item.document_details.rejected_by_user_code} <br> ${item.document_details.rejected_by_user_name}  <br><span > ${item.document_details.rejected_on_datetime}</span></td>`
                            } else {
            ...
            //            row+=`<td style="white-space:nowrap">
            // <button title="History" class="btn_grey_c"><a href="../user/masters/trucks/documents/document-history?truck_eid=${dd}&document_type_eid=${item.type_eid}&document_name=${item.type_name}"><i class="fa fa-history"></i></a></button>
            // <button title="Notes" class="btn_grey_c" onclick="open_notes({url:'../user/miscellaneous/notes/details?reference=TRUCK-DOCUMENT&eid=${dd}&document-type-eid=${item.type_eid}'})"><i class="fa fa-sticky-note"></i></a></button></td>`
                            row += `</tr>`
...
<!-- ------------------------------TRUCK DOCUMENTS SECTION END HERE------------------------------------------------------------------------------------------------------------- -->
