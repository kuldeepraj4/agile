                ...
                <div>
                    ...
                    <div class="field-section single-column">
                        ...
                        <select id="mySelect" name="payment_status" required value="" data-optional >
                            <option <?php echo ($details['payment_status'] == 'PAID') ? 'selected': '' ?> value="PAID">Paid</option>
                            <option <?php echo ($details['payment_status'] == 'UNPAID') ? 'selected': '' ?> value="UNPAID">Unpaid</option>
                        </select>
                        </div>
                        <div class="field-p">
                            <label>Payment Mode</label>
                            <select class="removereq mode1" name="payment_mode_id" id="payment_mode_id" type="text" required data-optional data-default-select="<?php echo $details['payment_mode_id'] ?>"></select>
                        </div>
                        <div class="field-p">
                            <label>Payment Ref No</label>
                            <input class="removereq mode2" name="payment_ref_no" type="text" required value="<?php echo $details['payment_ref_no'] ?>" data-optional >
                        </div>
                        <div class="field-p">
                            <label>Invoice No</label>
                            <input class="inv_no" name="invoice_no" type="text" required value="<?php echo $details['invoice_no'] ?>" data-optional>
                        </div>
                        <div class="field-p">
                            <label>Date Paid</label>
                            <input class="removereq mode3" name="payment_date" type="text" data-date-picker="" required value="<?php echo $details['payment_date'] ?>" data-optional>
                        </div>

                        <div class="field-p">
                            <label>Payment Notes</label>
                            <textarea name="payment_remarks" style="height: 80px"><?php echo $details['payment_remarks'] ?></textarea>
                        </div>
                    
                        <div class="field-p">
                            <label>Received By</label>
                            <select name="received_by" data-default-select="<?php echo $details['received_by'] ?>"></select>
                        </div>
                        <div class="field-p">
                            <label>Receipt Date</label>
                            <input name="receipt_date" type="text" data-date-picker="" required value="<?php echo $details['receipt_date'] ?>" data-optional>
                        </div>
                    ...
                    <div class="field-section table-rows">
                        <table style="width: 100%">
                            ...
                            <tfoot>
                                ...
                                <tr>
                                <td colspan="5" style="padding-right:50px;font-weight: bold;"><span id="total" data-total-amount>Total: 0</span></td>
        ...
        if(isValidForm){
            ...
            $.ajax({
                ...
                success:function(data){
                 ...
                 alert(data.message);
                 if(data.status){
                    location.href="../user/inventory/receipts"
    ...
    function update() {
        ...
        $data_item_rows.each(function(index) {
            ...
            if ($data_item_row.find("[name=item_location_id_fk]").val()) {
                var stop_row = {
                    receipt_detail_id: $data_item_row.find("[name=receipt_detail_id]").val(),
                    receipt_detail_id_status: $data_item_row.find("[name=receipt_detail_id_status]").val(),
                    item_location_id_fk: $data_item_row.find("[name=item_location_id_fk]").val(),
                    quantity: $data_item_row.find("[name=quantity]").val(),
                    old_quantity: $data_item_row.find("[name=old_quantity]").val(),
        ...
        if (isValidForm) {
            ...
            var obj = {
                ...
                po_id_fk: <?php echo $details['po_id_fk'] ?>,
                amount: total_amount,
                ...
                receipt_date: $('[name="receipt_date"]').val(),
...
<script type="text/javascript">
    ...
    {
        var items_list = '<?php echo json_encode($details['po_details']['items']) ?>';
...
<script type="text/javascript">
    ...
    function add_row(param) {
        ++counter;
        if (param.hasOwnProperty('receipt_detail_id')) {
            receipt_detail_id = param.receipt_detail_id
        } else {
            receipt_detail_id = ''
        }
        if (param.hasOwnProperty('receipt_detail_id_status')) {
            receipt_detail_id_status = param.receipt_detail_id_status
        } else {
            receipt_detail_id_status = 'ACT'
        }
        ...
        if (param.hasOwnProperty('default_old_quantity')) {
            default_old_quantity = param.default_old_quantity
        } else {
            default_old_quantity = ''
        }
        ...
        var $add_row = `<tr id="row_${counter}" data-item-row>
            ...
            <td>
                ...
                <input type="hidden" name="receipt_detail_id_status" value="${receipt_detail_id_status}" >
                <select required style="width: 200px" name="item_location_id_fk" data-calulate-amount></select>
            </td>
            <td>
                <input type="hidden" name="old_quantity" value="${(default_old_quantity) ? default_old_quantity : 0 }">
                <input required min="1" style="width: 100px" name="quantity" type="number" value="${(default_quantity) ? default_quantity : 1 }" data-calulate-amount>
            </td>
            <td>
                <input required min="0" max="100" style="width: 100px" name="rate" type="number" value="${(default_rate) ? default_rate : 0 }" data-calulate-amount>
            </td>
            <td>
                <input required min="0" style="width: 100px" name="tax" type="number" value="${(default_tax) ? default_tax : 0 }" data-calulate-amount>
            </td>
            <td>
    ...
    get_items(<?php echo $details['po_details']['location_id_fk'] ?>);
    ...
    $(document.body).on('click', '[data-remove-row]', function() {
        let row_id=$(this).parents('tr').attr('id');
        if($('tr#'+row_id+' [name="receipt_detail_id"]').val()){
            $('tr#'+row_id+' [name="receipt_detail_id_status"]').val('DEL');
...
<script type="text/javascript">
    async function f() {
        ...
        $.each(items_list, function(index, item) {
            add_row({
                receipt_detail_id: item.receipt_detail_id,
                receipt_detail_id_status: item.receipt_detail_id_status,
                default_select_item_location_id_fk: item.item_location_id_fk,
                default_quantity: item.quantity,
                default_old_quantity: item.quantity,
    ...
    quick_list_users().then(function(data) {
      ...
      if (data.status) {
        ...
        if (data.response.list) {
          ...
          select_default('[name="received_by"]')
        }
      }
    }).catch(function(err) {
