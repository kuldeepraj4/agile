...
<section class="list-200 content-box" style="margin: auto;max-width: 500px">
    <h1 class="list-200-heading">Inventory - Storage Types</h1>
    ...
    <section class="list-200-top-action">
        <div class="list-200-top-action-left">
            ...
            <div class="filter-item-full">
                <label>Search</label>
                <input type="text" placeholder="Storage Type / Status" data-filter="search" onkeyup="goto_page(1)">
                </select>
            </div>
        ...
        <div class="list-200-top-action-right">
            <div>
                ...
                if (in_array('P0406', USER_PRIV)) {
                    echo "<button class='btn_grey button_href'><a href='../user/inventory/masters/storage-type/add-new'>Add New</a></button>";
               }
                ?>
            </div>
        </div>
    ...
    <div class="table  table-a">
        <table>
            <thead>
                ...
                <tr>
                    <th>Sr. No.</th>
                    <th data-table-sort-by="type">Storage Types</th>
...
<script type="text/javascript">
    function show_list() {
        ...
        get_storage_types(param).then(function(data) {
            ...
            if (data.status) {
                ...
                if (data.response.list) {
                    ...
                    $.each(data.response.list, function(index, item) {
                            ...
                            row += `<td>` + item.storage_type + `</td>`;
                            row += `<td>` + item.storage_type_status + `</td>`;
                            row += `<td>`;
                            <?php if (in_array('P0408', USER_PRIV)) {
                            ?>
                                 row += `<button title="Edit" class="btn_grey_c"><a href="../user/inventory/masters/storage-type/update?eid=` + item.eid + `"><i class="fa fa-pen"></i></a></button>`;
                            <?php
                            } ?>
