...
<section class="form-a" style="max-width: 600px">
  <div class="form-a-header">UPDATE NEW EARNING/DEDUCTION</div>
  <form method="POST" id="MyForm" onsubmit="return update()">
    ...
    <div>
      <label>Amount</label>
      <input type="text" name="amount" value="<?php echo $data['details']['amount']; ?>" pattern="[0-9.]{1,}" required="required">
    </div>
    <div class="field-p">
      <label>Remarks</label>
      <textarea name="remarks" style="height: 80px;"><?php echo $data['details']['remarks']; ?></textarea>
    </div>
    <div>
...
<script type="text/javascript">
  ...
  if(data.status){
    ...
    if(data.response.list){
      ...
      $('[name="parameter_id"] option[value="<?php echo $data['details']['parameter_id']; ?>"]').prop('selected',true);     
    }
  }
}).catch(function(err) {
