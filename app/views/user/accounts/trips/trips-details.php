...
<section class="action-button-box" >
  <?php 
  echo   (in_array($details['approval_status'],array("PENDING","CANCELLED","REJECTED")) && in_array('P0121', USER_PRIV))?'<button type="button" class="btn_blue"><a href="../user/accounts/trips/update?eid='.$details['eid'].'">UPDATE</a></button> ':""; 
  echo   ($details['is_paid']==false && $details['approval_status']=="APPROVED" && in_array('P0144', USER_PRIV))?'<button type="button" data-action="cancel" class="btn_red">CANCEL</button> ':"";
  echo   ($details['is_paid']==true && $details['approval_status']=="APPROVED" && in_array('P0149', USER_PRIV))?'<button type="button" class="btn_red"><a href="../user/accounts/trips/resettle?eid='.$details['eid'].'">RESETTLE</a></button> ':"";
  ?>
</section>
<br>
<section class="list-200 content-box" style="margin: auto;max-width: 1200px">
  <h1 class="list-200-heading">Trip Details</h1>
        ...
        <section class="content-box" style="margin: auto;width:550px">
          ...
          <div class="list-multi-desc">
            ...
            <section>
              <h2>BASIC</h2>
              <div>
                <div><p>Basic Earnings <br>( MILES x RATE ) <?php echo $ts['miles'].' x '.$ts['pay_per_mile']; ?></p><p><?php echo $ts['basic_earnings']; ?></p></div>
              </div>
            </section>
                ...
                <section>
                  ...
                  <div>
                    ...
                    foreach ($ts_sp['parameters'] as $ts_sp_param) {
                      ?>
                      <div><p><?php echo $ts_sp_param['name']; ?></p><p><?php echo $ts_sp_param['amount']; ?></p></div>
                      <small style="padding: 10px"><b>Remarks:-</b> <?php echo $ts_sp_param['remarks']; ?></small>
                      <br><br>
                      <?php
                    }
            ...
            ?>
            <h3><p>Net Payment</p><p><?php echo $ts['net_earnings']; ?></p></h3>

            <section>
              <h2>INCENTIVES</h2>
              <div>
                <div><p>MILES x INCENTIVES PER MILE<br><?php echo $ts['miles'].' x '.$details['incentive_per_mile']; ?></p><p><?php echo $ts['incentive']; ?></p></div>
              </div>
            </section>
            ...
            <section>
              <h2>Remarks</h2>
              <div>
                <div><p style="white-space: pre;"><?php echo $ts['remarks'] ?></p></div>
              </div>
            </section>  
...
show_notes({notes_area:'first',reference_eid:'<?php echo $details['eid']?>'})
...
var myVar;    
    function showTime(){
show_notes({notes_area:'first',reference_eid:'<?php echo $details['eid']?>',last_note_eid:last_note_eid})
    }
...
<br>
<div style="text-align:center;"><button type="button" class="btn_green" onclick="back_alert()">Back to Main Page</button></div>
<script type="text/javascript">
  function back_alert(){
